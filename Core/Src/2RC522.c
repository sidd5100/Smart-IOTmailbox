...
/*uint16_t MFRC522_GetCSPin(void)
{
    return (mode) ? GPIO_PIN_4 : GPIO_PIN_3;
}*/

/*
 * Function Name: RC522_2_SPI_Transfer
...
/*
 * Function Name: MFRC522_2_ToCard
...
/*
 * Function Name: MFRC522_2_Request
...
/*
 * Function Name: MFRC522_2_Anticoll
...
/*
 * Function Name: MFRC522_2_SelectTag
 ...
 * Return value: the successful return MI_OK
 */
uchar MFRC522_2_Auth(uchar authMode, uchar BlockAddr, uchar *Sectorkey, uchar *serNum)
{
    uchar status;
    uint recvBits;
    uchar i;
    ...
    status = MFRC522_2_ToCard(PCD_AUTHENT, buff, 12, buff, &recvBits);

    if ((status != MI_OK) || (!(Read_2_MFRC522(Status2Reg) & 0x08)))
    {
		status = MI_ERR;
	}
...
/*
 * Function Name: MFRC522_2_Read
 * Description: Read block data
 * Input parameters: blockAddr - block address; recvData - read block data
 * Return value: the successful return MI_OK
 */
uchar MFRC522_2_Read(uchar blockAddr, uchar *recvData)
{
    uchar status;
    uint unLen;
    ...
    status = MFRC522_2_ToCard(PCD_TRANSCEIVE, recvData, 4, recvData, &unLen);
...
uchar MFRC522_2_Write(uchar blockAddr, uchar *writeData)
{
    ...
    status = MFRC522_2_ToCard(PCD_TRANSCEIVE, buff, 4, buff, &recvBits);
    ...
    if (status == MI_OK)
    {
        ...
        status = MFRC522_2_ToCard(PCD_TRANSCEIVE, buff, 18, buff, &recvBits);
